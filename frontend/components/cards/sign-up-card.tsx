...
import { singUpFormSchema } from "@/types/userSchema";
...
const SignUpCard = () => {
  ...
  const onSubmit = async (data: z.infer<typeof singUpFormSchema>) => {
    let profileImageUrl = "";
    try {
      toast({
      ...
      if (data.profileImage && data.profileImage.length > 0) {
        const response = await uploadImage(data.profileImage[0]);
      ...
      if (token) {
        ...
        toast({
          ...
          description: "Welcome to ResumeRise.",
        });
              ...
              <div className="flex flex-col gap-5">
                <div className="flex flex-col items-center text-center space-y-1">
                  ...
                  <p className="text-balance text-sm text-muted-foreground">
                    Get started with AI-optimized resumes for free
                  </p>
                </div>

                {/* Profile Image Upload */}
                <ProfileImageUpload form={form} name="profileImage" />
